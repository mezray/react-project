...
export default function MainPage() {
  ...
  const addTask = () => {
    ...
    if(newTaskName && newTaskPrice && payer && payees) {
      setTasks([...tasks, { name: newTaskName, price: newTaskPrice, payer, payees }]);
  ...
  const editTask = (index) => {
    ...
    if(newTaskName && newTaskPrice && payer && payees) {
      ...
      newTasks[index] = { name: newTaskName, price: newTaskPrice, payer, payees };
      setTasks(newTasks);
      updateDebtList(payer, payees, newTaskPrice);
    }
  };
    ...
    <>
      ...
      <div>  
        ...
        {tasks.map((task, index) => (
            <li key={index}>
                {task.name} - ${task.price} - Paid by: {task.payer} - Payees: {(task.payees || []).join(', ')}
